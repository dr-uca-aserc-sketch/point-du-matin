...
async function ask(prompt, maxTokens = 16000) {
  const text = API_KEY
...
async function composeDaily(euDigest, intlDigest) {
  ...
  return {
    head,
    desks: (partA.desks || []).concat(partB.desks || [])
  };
}
...
async function main() {
  ...
  if (!edition.mustReads.length || !desks.length) throw new Error("model returned an incomplete edition");
